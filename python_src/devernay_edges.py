...
class DevernayEdges:
    ...
    def detect_edges(self):
        if self.sigma == 0.0:
            ...
            self.image_filtered = self.image.filter(ImageFilter.GaussianBlur(radius = self.sigma))
            self.image_vec = np.ravel(np.array(self.image_filtered, dtype=np.float64)).reshape([self.img_x*self.img_y, 1])
            self.compute_gradient()
